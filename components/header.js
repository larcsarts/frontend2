...
class Header extends React.Component {
  ...
  hoverLink = e => {
    ...
    this.hoverBox.current.style.left = e.target.offsetLeft - 5 + 'px';
    this.hoverBox.current.style.top =
      e.target.offsetTop + e.target.offsetHeight - 3 + 'px';
    this.hoverBox.current.style.width = e.target.offsetWidth + 10 + 'px';
  };
  ...
  hoverOutLink = () => {
    ...
    if ([6, 1, 2, 3, 10, 4, 8].includes(parseInt(this.state.menu))) {
      ...
      this.hoverBox.current.style.left = elem.offsetLeft - 5 + 'px';
      this.hoverBox.current.style.top =
        elem.offsetTop + elem.offsetHeight - 3 + 'px';
      this.hoverBox.current.style.width = elem.offsetWidth + 10 + 'px';
    } else {
            ...
            <nav ref={this.nav}>
              ...
              <Link prefetch href="/">
                <a
                  ...
                  className={`header__button ${menu == 1 &&
                    this.state.classSelected}`}
                >
                  LIVRO DE ORDENS
                </a>
